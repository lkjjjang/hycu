...
<script type="text/javascript">
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-12">${detail.bbsContente}</div>
		</div>
		<div class="row">
			<div class="col-12" style="text-align: center;">
				<c:if test="${userID == 'guest'}">
					<!-- ${userID} 로그인시 세션에 저장해둠 -->
					<button onclick="guest()" class="btn btn-primary">${detail.bbsUpvote} 추천</button><p>
				</c:if>
				<c:if test="${userID != 'guest'}">
					<!-- ${userID} 로그인시 세션에 저장해둠 -->
					<button onclick="upvoteUpdate(${detail.bbsID}, ${userID})" class="btn btn-primary">${detail.bbsUpvote} 추천</button><p>
				</c:if>
			</div>
		</div>
		<div class="row">
