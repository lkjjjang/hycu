	...
	<nav class="navbar navbar-expand-lg navbar-light bg-light">
		...
		<div id="navbar" class="collapse navbar-collapse">
			<ul class="navbar-nav mr-auto">
				...
				<li class="nav-item">
					<a class="nav-link" href="freeBoardListController?pageNumber=1">잡담게시판</a>
				</li>
				<li class="nav-item">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="evaluationUpdateController" method="post">
						<div class="form-row">
							<div class="form-group col-sm-6">
								<label>강의명</label>
								<input type="hidden" name="evaluationID" value="${evaluationID}">
								<input type="text" name="lectureName" class="form-control" value="${lectureName}" maxlength="20">
							</div>
							<div class="form-group col-sm-6">
								<label>교수명</label>
								<input type="text" name="professorName" class="form-control" value="${professorName}" maxlength="20">
							</div>
						</div>
						<div class="form-row">
							...
							<div class="form-group col-sm-4">
								...
								<select name="semesterDivide" class="form-control">
									<c:set var="semesterDivide" value="${semesterDivide}"></c:set>
									<c:if test="${semesterDivide eq '1학기'}">
										<option value="1학기" selected>1학기</option>
										<option value="여름학기">여름학기</option>
										<option value="2학기">2학기</option>
										<option value="겨울학기">겨울학기</option>
									</c:if>
									<c:if test="${semesterDivide eq '여름학기'}">
										<option value="1학기">1학기</option>
										<option value="여름학기" selected>여름학기</option>
										<option value="2학기">2학기</option>
										<option value="겨울학기">겨울학기</option>
									</c:if>
									<c:if test="${semesterDivide eq '2학기'}">
										<option value="1학기">1학기</option>
										<option value="여름학기">여름학기</option>
										<option value="2학기" selected>2학기</option>
										<option value="겨울학기">겨울학기</option>
									</c:if>
									<c:if test="${semesterDivide eq '겨울학기'}">
										<option value="1학기">1학기</option>
										<option value="여름학기">여름학기</option>
										<option value="2학기">2학기</option>
							...
							<div class="form-group col-sm-4">
								...
								<select name="lectureDivide" class="form-control">
										<c:set var="lectureDivide" value="${lectureDivide}"></c:set>
										<c:if test="${lectureDivide eq '전공'}">
											<option value="전공" selected>전공</option>
											<option value="교양">교양</option>
											<option value="기타">기타</option>
										</c:if>
										<c:if test="${lectureDivide eq '교양'}">
											<option value="전공">전공</option>
											<option value="교양" selected>교양</option>
											<option value="기타">기타</option>
										</c:if>
										<c:if test="${lectureDivide eq '기타'}">
											<option value="전공">전공</option>
											<option value="교양">교양</option>
						...
						<div class="form-group">
							<label>제목</label>
							<input type="text" name="evaluationTitle" value="${evaluationTitle}" class="form-control" maxlength="30">
						</div>
						<div class="form-group">
							<label>내용</label>
							<textarea name="evaluationContent" class="form-control" maxlength="2048" style="height: 180px;">${evaluationContent}</textarea>
						</div>
						<div class="form-row">
							<div class="form-group col-sm-3">
								<label>종합</label>
								<select name="totalScore" class="form-control">
									<c:set var="totalScore" value="${totalScore}"></c:set>
									<c:if test="${totalScore eq 'A'}">
