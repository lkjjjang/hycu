	...
	<div class="container">
		<h1>
			...
			<table class="table table-hover">
				...
				<tbody>
					<c:forEach var="devStory" items="${devStory}" begin="0" end="${freeBBSListPrintCount}">
						<tr class="row">
							<td class="col-2" style="text-align: center;">
							<!-- boardID, boardTitle, boardPassword, boardRegDate, cmt_count, use_file --> 
								<c:if test="${userID == 'admin'}">
									<input type="checkbox" name="delCheck_id" id="delCheck_id" value="${devStory.boardID}">
								</c:if>
								${devStory.boardID} 
							</td>
							<td class="col-6">
								<c:if test="${fn:length(devStory.boardTitle) > 30}">
									<a class="text-dark" href="devStoryDetailController?id=${devStory.boardID}">
										${fn:substring(devStory.boardTitle, 0, 30)}...
									</a>
								</c:if>
								<c:if test="${fn:length(devStory.boardTitle) <= 30}">
									<a class="text-dark" href="devStoryDetailController?id=${devStory.boardID}">
								...
								<c:if test="${devStory.cmt_count != 0}">
									<span style="color: #00bfff;">[${devStory.cmt_count}]</span>
								</c:if>	
								<c:if test="${devStory.use_file != 0}">
									<img src="images/imageIcon.png">
								</c:if>				
							</td>
							<td class="col-2" style="text-align: center;">관리자</td>
							<td class="col-2" style="text-align: center;">${devStory.boardRegDate}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>	
