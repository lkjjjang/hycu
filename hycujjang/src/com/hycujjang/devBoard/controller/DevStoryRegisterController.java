...
@WebServlet("/devStoryRegisterController")
public class DevStoryRegisterController extends HttpServlet {
	private String backURL = "devStoryListController?pageNumber=1";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		DevBoardDTO devBoardDTO = new DevBoardDTO(title, password, modifydeContent);
		int result = devBoardDAO.write(devBoardDTO);
		int imageUpdateResult = 0;
		
		// 이미지정보 추가
		DevFileDAO devFileDAO = new DevFileDAO();	
		if (usedFileList.size() != 0 && reNameList.size() != 0) {
			int boardID = devBoardDAO.getBoardID(devBoardDTO);
			
			for (int i = 0; i < usedFileList.size(); i++) {
				int fileID = devFileDAO.getFileID(userID, usedFileList.get(i));
				DevFileDTO fileDTO = new DevFileDTO(fileID, boardID, usedFileList.get(i), reNameList.get(i));
				imageUpdateResult = devFileDAO.updateFileName(fileDTO);
			}
		}
