...
<body>
	...
	<script type="text/javascript">
		function commentRemove(){
			// 전달되는 파라미터값 형태
			// delID_ comment or reply or bbsContent or contentModify _ id
			var param = getParam('id').split('_');
			var divide = param[1];
			var id = param[2];
			var password = $('#delPassword').val();
			if (divide == 'contentModify') {
		...
		function passCheck(bbsId, password) {
			var data = {id: bbsId, password: password}
			$.ajax({
				type: "post",
				url: 'contentModify',
				...
				success: function(json) {
					...
					if (result == 'ok') {
						opener.goModify(bbsId);
					} else {
						opener.modifyMsg('비밀번호가 틀렸습니다.');
					}
					window.close();
				},
				error: function(json) {
